...
/**
 * MARC Protocol — LangChain Tool Plugin
 *
 * Provides 5 LangChain-compatible tools for FHE confidential USDC operations:
 * - MarcWrapTool:     Wrap plaintext USDC into FHE-encrypted cUSDC (ERC-7984)
 * - MarcUnwrapTool:   Initiate cUSDC unwrap back to plaintext USDC
 * - MarcTransferTool: Send encrypted cUSDC to another address
 * - MarcBalanceTool:  Query encrypted balance handle
 * - MarcPayTool:      Full x402 payment flow (fetch 402, pay, verify, retry)
 *
 * Usage:
 *
 *   import { createMarcTools, MARC_SEPOLIA_ADDRESSES } from "@marc-protocol/langchain-tool";
 *
 *   const tools = createMarcTools(signer, fhevmInstance);
 *   // tools is an array of LangChainTool objects ready for an LLM agent
 */
 ...
 *
 * LangChain tools accept a JSON string as input and return a string result.
 * The schema describes the expected input shape for the LLM.
 */
export interface LangChainTool {
  /** Tool name (snake_case, used by LLM for selection) */
  name: string;
  /** Human-readable description — tells the LLM when to use this tool */
  description: string;
  /** JSON Schema describing the expected input parameters */
  schema: Record<string, any>;
  /** Execute the tool with a JSON string input, returns a string result */
...
function safeParseInput<T>(input: string): T {
  try {
    return JSON.parse(input) as T;
  } catch {
    throw new MarcToolError(`Failed to parse input JSON: ${input.slice(0, 200)}`);
  }
}
...
export class MarcWrapTool implements LangChainTool {
  name = "marc_wrap";
  description =
    ...
    "Input requires amount (in raw USDC units, 6 decimals — 1000000 = 1 USDC) and optionally a recipient address.";
  ...
  async call(input: string): Promise<string> {
    const parsed = safeParseInput<{ amount: string; to?: string }>(input);
    ...
    const to = parsed.to ?? signerAddress;
...
export class MarcUnwrapTool implements LangChainTool {
  name = "marc_unwrap";
  description =
    ...
    "Input requires amount (in raw USDC units, 6 decimals). Requires FhevmInstance for encryption.";
  ...
  async call(input: string): Promise<string> {
    ...
    const parsed = safeParseInput<{ amount: string; from?: string }>(input);
    ...
    const signerAddress = await this.signer.getAddress();
    const from = parsed.from ?? signerAddress;
...
export class MarcTransferTool implements LangChainTool {
  name = "marc_transfer";
  description =
    ...
    "Input requires to (recipient address) and amount (raw USDC units, 6 decimals). Requires FhevmInstance.";
  ...
  async call(input: string): Promise<string> {
    ...
    const parsed = safeParseInput<{ to: string; amount: string }>(input);

    const amount = BigInt(parsed.amount);
    assertPositiveAmount(amount, "amount");
    assertValidAddress(parsed.to, "to");
    assertValidAddress(this.addresses.tokenAddress, "tokenAddress");
    ...
    const token = await createContract(this.addresses.tokenAddress, TOKEN_ABI as unknown as string[], this.signer);
    const tx = await token.confidentialTransfer(parsed.to, encrypted.handles[0], encrypted.inputProof);
    const receipt = await tx.wait();
...
export class MarcBalanceTool implements LangChainTool {
  name = "marc_balance";
  description =
    ...
    "Input optionally takes an address (defaults to signer address).";
  ...
  async call(input: string): Promise<string> {
    const parsed = safeParseInput<{ address?: string }>(input);

    assertValidAddress(this.addresses.tokenAddress, "tokenAddress");

    const signerAddress = await this.signer.getAddress();
    const address = parsed.address ?? signerAddress;
...
export class MarcPayTool implements LangChainTool {
  name = "marc_pay";
  description =
    ...
    "Input requires url (the protected resource). Optionally maxPayment (safety cap in raw USDC units).";
  ...
  async call(input: string): Promise<string> {
    ...
    const parsed = safeParseInput<{ url: string; maxPayment?: string }>(input);
    ...
    try {
      ...
      throw new MarcToolError("Failed to parse 402 response body", { url: parsed.url });
    }
    ...
    const maxPayment = parsed.maxPayment ? BigInt(parsed.maxPayment) : undefined;
    ...
    try {
      const retryRes = await fetch(parsed.url, {
...
/**
 * Create all 5 MARC Protocol tools for LangChain.
 *
 ...
 * @returns Array of LangChainTool objects
 */
export function createMarcTools(
  signer: Signer,
  fhevmInstance?: FhevmInstance,
  config?: MarcToolConfig
): LangChainTool[] {
  return [
    new MarcWrapTool(signer, fhevmInstance, config),
    new MarcUnwrapTool(signer, fhevmInstance, config),
    new MarcTransferTool(signer, fhevmInstance, config),
    new MarcBalanceTool(signer, fhevmInstance, config),
    new MarcPayTool(signer, fhevmInstance, config),
  ];
}
